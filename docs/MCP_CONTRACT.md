...
El endpoint se descubre mediante `McpHttpServerDefinition` con la etiqueta corta estable `GOVP AW` y debe ser HTTPS con ruta exacta `/mcp`. La extensión solo selecciona herramientas bajo el namespace exacto configurado en `govp.mcpProviderNamespace`; el valor predeterminado compatible con la implantación actual es `mcp_govp_implemen`. También se admite una herramienta exacta únicamente si lleva la etiqueta de proveedor configurada en su descripción. No hay fallback.
